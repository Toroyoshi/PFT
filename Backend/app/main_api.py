...
# Cria TODAS as tabelas no PostgreSQL (incluindo Alertas) se não existirem no arranque
Base.metadata.create_all(bind=engine)

app = FastAPI(title="Corner Anti-Theft Enterprise API", version="3.0 - Offline-First Ready")
...
BASE_DIR = Path(__file__).resolve().parent.parent.parent
METRICAS_DIR = os.path.join(BASE_DIR, 'Metricas')  # Mantido para o stream de vídeo em tempo real
...
@app.get("/")
def read_root():
    return {"status": "A Corner Enterprise API está a correr a 100% com PostgreSQL unificado!"}


# ============ SCHEMAS LOCAIS ============

class AlertaSincronizado(BaseModel):
    """Schema para validar o payload JSON enviado pelo Dispatcher do Nó de Borda"""
    track_id: int
    tipo_alerta: str
    confianca: float
    timestamp: str
...
# ============ ENDPOINTS DE ALERTAS (PostgreSQL) ============

@app.get("/api/alertas/recentes")
def get_recent_alerts(db: Session = Depends(get_db)):
    """Obtém os 10 alertas de furto mais recentes diretamente do PostgreSQL (Para o Dashboard)."""
    try:
        # Consulta rápida, limpa e ordenada pelo motor SQL
        alertas_recentes = db.query(AlertaModel).order_by(AlertaModel.timestamp.desc()).limit(10).all()
        return {"alertas": alertas_recentes}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro ao ler histórico de alertas no PostgreSQL: {str(e)}")

@app.post("/api/alertas/sincronizar")
def sync_alerta_from_edge(alerta: AlertaSincronizado, db: Session = Depends(get_db)):
    """Recebe alertas do Dispatcher local do Nó de Borda e injeta-os de forma segura no PostgreSQL."""
    try:
        novo_alerta = AlertaModel(
            track_id=alerta.track_id,
            tipo_alerta=alerta.tipo_alerta,
            confianca=alerta.confianca,
            # Converte a string ISO do Dispatcher para objeto DateTime compatível com a BD
        ...
        db.add(novo_alerta)
        db.commit()
        return {"status": "sucesso", "mensagem": "Consistência assegurada. Alerta gravado na Cloud."}
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Erro de Injeção no PostgreSQL: {str(e)}")


# ============ ENDPOINTS DE MÉTRICAS (PostgreSQL) ============
...
def register_metrics(metrica: MetricaNodeCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        db.add(nova_metrica)
        db.commit()
        return {"status": "sucesso", "mensagem": f"Métricas registadas no PostgreSQL para {metrica.node_id}"}
    except Exception as e:
        db.rollback()
        raise HTTPException(status_code=500, detail=f"Erro ao registar na BD: {str(e)}")
...
# ============ ENDPOINTS DE VIDEO/STREAM (Live Feed Binário) ============

@app.get("/api/video/frame")
def get_current_frame():
    frame_path = os.path.join(METRICAS_DIR, 'last_frame.jpg')
    if not os.path.exists(frame_path):
        return {"erro": "Nenhum frame disponível", "mensagem": "O Edge precisa de estar a correr."}
    try:
        return FileResponse(frame_path, media_type="image/jpeg")
    except Exception as e:
        return {"erro": f"Erro ao servir o frame: {str(e)}"}
